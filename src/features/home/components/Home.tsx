  ...
  const guidelineSections = [
    {
      ...
      content: (
        <>
          <Paragraph>Ensure developers can install, develop, and build the project.</Paragraph>
          <ul>
            <li><Text code>pnpm install</Text> - Install dependencies</li>
            <li><Text code>pnpm dev</Text> - Start the development server</li>
            <li><Text code>pnpm build</Text> - Build the production bundle</li>
          </ul>
        </>
      ),
    },
    ...
    },
    {
      ...
      content: (
        <>
          <Paragraph>Organize routing via centralized and feature-specific route files.</Paragraph>
          <ul>
            <li>Centralized routing logic is in <Text code>src/routes/index.tsx</Text></li>
            <li>Each feature exports its own route(s) via <Text code>routes.tsx</Text> inside the feature folder</li>
    ...
    },
    {
      ...
      content: (
        <>
          <Paragraph>Follow official MirageJS structure for robust and type-consistent mocks.</Paragraph>
          <ul>
            <li>All Mirage configuration must reside in <Text code>src/mirage/</Text></li>
            <li>Separate files must exist for <Text code>models.ts</Text>, <Text code>factories.ts</Text>, <Text code>seeds.ts</Text>, and <Text code>routes.ts</Text></li>
            <li>Define models in <Text code>models.ts</Text> using Mirage's <Text code>Model</Text></li>
            <li>Use <Text code>factories.ts</Text> to generate mock data with Faker or sample data</li>
            <li>Use <Text code>seeds.ts</Text> to populate mock DB at server boot time</li>
            <li>Define REST-style endpoints in <Text code>routes.ts</Text> using <Text code>this.namespace = 'api'</Text></li>
            <li>All mock data must conform to shared types from <Text code>src/common/types/</Text></li>
            <li>Extend Mirage's TypeScript support in <Text code>mirage.d.ts</Text></li>
            <li>Initialize Mirage in <Text code>main.tsx</Text> only if the env variable <Text code>VITE_USE_MOCKS=true</Text></li>
            <li>Use <Text code>environment: 'test'</Text> in test suites</li>
    ...
    },
    {
      ...
      content: (
        <>
          <Paragraph>Use environment variables consistently for mocks, endpoints, and feature flags.</Paragraph>
          <ul>
            <li>Define all environment variables with <Text code>VITE_</Text> prefix (Vite convention) in <Text code>.env</Text> files</li>
            <li>Use <Text code>VITE_API_BASE_URL</Text> for all HTTP calls</li>
            <li>Use <Text code>VITE_USE_MOCKS</Text> to conditionally start Mirage</li>
            <li>Access environment variables via <Text code>import.meta.env.VITE_*</Text> with proper fallback logic</li>
            <li>Avoid hardcoding values like base URLs or mocking flags directly in code</li>
            <li>All environment variable keys should be documented in <Text code>.env.example</Text></li>
          </ul>
        </>
      ),
    },
    {
      ...
      content: (
        <>
          <Paragraph>Follow standardized testing practices with React Testing Library + Jest.</Paragraph>
          <ul>
            <li>Use React Testing Library and Mirage for integration-style tests</li>
            <li>Each feature must have a <Text code>tests/</Text> folder</li>
            <li>Mock backend behavior in tests via Mirage (with proper seeding)</li>
            <li>Run tests with <Text code>pnpm test</Text>. Use <Text code>-- &lt;path&gt;</Text> to target specific files</li>
            <li>Use <Text code>screen</Text>, <Text code>waitFor</Text>, and <Text code>userEvent</Text> for clean test interaction logic</li>
          </ul>
        </>
      ),
    },
    {
      ...
      content: (
        <>
          <Paragraph>Use Ant Design as the primary UI component library for consistent user experience.</Paragraph>
          <ul>
            <li>Always use Ant Design components instead of HTML elements when an equivalent component exists</li>
            <li>Import components individually to optimize bundle size: <Text code>import {'{ Button }'} from 'antd'</Text> not <Text code>import * from 'antd'</Text></li>
            <li>Use Ant Design's layout components (<Text code>Layout</Text>, <Text code>Row</Text>, <Text code>Col</Text>) for page structure</li>
            <li>Follow Ant Design's form patterns and validation approach</li>
            <li>Use Ant Design's <Text code>Typography</Text> components for text elements</li>
            <li>Customize theme through the <Text code>ConfigProvider</Text> in main.tsx only</li>
            <li>For custom styling, use CSS modules or styled-components with Ant Design's design tokens</li>
            <li>Refer to the <a href="https://ant.design/components/overview/" target="_blank" rel="noopener noreferrer">Ant Design documentation</a> for component usage guidelines</li>
          </ul>
        </>
      ),
    },
    {
      ...
      content: (
        <>
          <Paragraph>Maintain consistent code formatting and organization.</Paragraph>
          <ul>
            <li>Use the <Text code>@/</Text> alias for absolute imports from <Text code>src/</Text></li>
    ...
    <Layout>
      <Content style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
        <Typography>
          ...
          <Paragraph>
            This project follows a modular, feature-based architecture for React applications
            using Redux Toolkit, Redux-Saga, React Router, and MirageJS with Ant Design components.
          </Paragraph>
        </Typography>
        ...
        <Row gutter={24} style={{ marginTop: '24px' }}>
          ...
          <Col xs={24} md={10} lg={8}>
            <Card title="Directory Structure" style={{ marginBottom: '24px' }}>
              <pre style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', fontSize: '12px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                {directoryStructure}
              </pre>
            </Card>
          </Col>
